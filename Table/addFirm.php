...
foreach($form_data as $key => $value){
    if(empty($value)){
        $form_data[$key] = "NULL";
        unset($notEmptyData[$key]);
    }
    else if(is_numeric($value)){
        $form_data[$key] = intval($value);
        $notEmptyData[$key] = $form_data[$key];
    }
...
if(count($idArr) > 0){
    var_dump($notEmptyData);

    $keys = array_keys($notEmptyData);
    $values = array_values($notEmptyData);

    $notEmptyData["ids"] = substr($notEmptyData["ids"], 1, strlen($notEmptyData["ids"])-2);

    $params = "";

    for($i = 1; $i < sizeof($notEmptyData); $i++){
        $params .= $keys[$i] . "=" . $values[$i];
        if($i < sizeof($notEmptyData) -1){
            $params .= ", ";
        }
    }
    
    $sql = "update firm set ".$params . " where id in(" . $notEmptyData["ids"] .")";
    ...
    if ($check_result) {
        if (mysqli_num_rows($check_result) > 0) {
            ...
            $query = "insert into firm(active, name, surname, email, phone, subject_id, source, date_of_contact, date_of_2_contact, date_of_meeting, result, workshop, brigade, practice, cv, note)
                     ...
                     ", ".$form_data["email"].", ".$form_data["phone"].", ".$form_data["subject_id"].
